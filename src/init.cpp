...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/worldcoin.conf are parsed in qt/bitcoin.cpp's main()
...
{
    uiInterface.ThreadSafeMessageBox(str, _("Worldcoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Worldcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
bool AppInit2()
{
    ...
    // Worldcoin: Keep irc seeding on by default for now.
//    if (fTestNet)
//    {
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Worldcoin is probably already running."), GetDataDir().string().c_str()));
