...
*Setup instructions for a Gitian build of Worldcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Worldcoin
...
- [Building Worldcoin](#building-worldcoin)
...
Clone the git repositories for worldcoin and Gitian and then checkout the worldcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/worldcoin-project/worldcoin.git
cd worldcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the worldcoin repository
...
To build Worldcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the worldcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/worldcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/worldcoin-project/worldcoin
...
For example:
```bash
URL=https://github.com/user/worldcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit worldcoin=${COMMIT} --url worldcoin=${URL} ../worldcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit worldcoin=${COMMIT} --url worldcoin=${URL} ../worldcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit worldcoin=${COMMIT} --url worldcoin=${URL} ../worldcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/worldcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/worldcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/worldcoin-build.assert
```
...
[worldcoin/gitian.sigs](https://github.com/worldcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
